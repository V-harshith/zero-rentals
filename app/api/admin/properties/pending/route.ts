...
export async function GET(request: NextRequest) {
  try {
    ...
    const rateLimitKey = `admin:properties:pending:${request.headers.get('x-forwarded-for')?.split(',')[0]?.trim() || request.headers.get('x-real-ip') || 'unknown'}`
    ...
    if (error) {
      return NextResponse.json({ error: 'Failed to fetch pending properties' }, { status: 500 })
    }
